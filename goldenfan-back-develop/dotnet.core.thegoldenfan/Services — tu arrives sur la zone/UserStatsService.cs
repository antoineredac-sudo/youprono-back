    ...
    public class UserStatsService
    {
        ...
        public async Task<PaginationModel<UserListStatsResult>> ByUserTeamAsync(Guid userId, string teamId, int page=1, int limit=10)
        {
            ...
            foreach(var item in pagination.Page)
            {
                ...
                // L'adversaire est nommé par son nom officiel, mais les équipes
                // créées depuis la console n'en avaient pas : on retombe alors sur
                // leur nom d'usage plutôt que de renvoyer du vide, qui s'affichait
                // "vs null" chez le joueur.
                var adversaire = item.Match.AwayTeam.TeamId.Equals(teamId) ? item.Match.HomeTeam.Team : item.Match.AwayTeam.Team;
                newObj.Opponent = !string.IsNullOrWhiteSpace(adversaire.OfficialName)
                    ? adversaire.OfficialName
                    : (!string.IsNullOrWhiteSpace(adversaire.Name) ? adversaire.Name : (adversaire.ShortName ?? string.Empty));
